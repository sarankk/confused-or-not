...
class cnn_model(nn.Module):
	def __init__(self):
		...
		self.layer1 = nn.Sequential(
		    nn.Conv1d(4443, 32, kernel_size=2, stride=2, padding=1),
		    nn.BatchNorm1d(32),
		    nn.ReLU())
	...
	for epoch in range(num_epochs):
		...
		for phase in ['train', 'val']:
			...
			for i_batch, sampled_batch in enumerate(dataloaders[phase]):
				...
				print('preds', preds)
				print('check_lables', check_labels)
...
def get_cnn_model(X_train, X_val, X_test, Y_train, Y_val, Y_test):
	...
	size1, len1F, len1C  = X_test.shape
	size2, len2F, len2C = X_train.shape
	size3, len3F, len3C = X_val.shape

	# print(X_train.shape)
	# X_train = np.reshape(X_train, (size2, len1C, len1F))
	# X_val = np.reshape(X_val, (size3, len2C, len2F))
	# X_test = np.reshape(X_test, (size1, len3C, len3F))
	# print 
