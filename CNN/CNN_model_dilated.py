...
class cnn_model_dilated(nn.Module):
    def __init__(self):
        super(cnn_model_dilated, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv1d(Samples, 32, kernel_size=2, stride=2,dilation = 2, padding=1),
            nn.BatchNorm1d(32),
            nn.ReLU())
        self.layer2 = nn.Sequential(
            nn.Conv1d(32, 16, kernel_size=2, stride=2, dilation = 2,padding=1),
            nn.BatchNorm1d(16),
            nn.ReLU())
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i_batch, sampled_batch in enumerate(dataloaders[phase]):
                ...
                if phase == 'train':
                    print zip(preds.numpy(), check_labels.numpy())
                    loss.backward()
                    optimizer.step()
                else :
                    print 'val'
               ...
               # print 'loss' , loss.data[0] * inputs.size(0)
               # print 'corrects' , (preds == check_labels).sum()
...
def get_cnn_model(X_train, X_val, X_test, Y_train, Y_val, Y_test):
    dataset_sizes = {}
    dataloaders = {}

    model = cnn_model_dilated()
    ...
    dataloaders['train'] = data_utils.DataLoader(train_data, batch_size=config.BATCH_SIZE, shuffle=False)

    val_data = data_utils.TensorDataset(X_val, Y_val)
    dataloaders['val'] = data_utils.DataLoader(val_data, batch_size=config.BATCH_SIZE, shuffle=False)
    ...
    Y_test_2 = Y_test.squeeze(1)

    test_loss = loss.data[0] * inputs.size(0)
    corrects = (preds == Y_test_2).sum()
